casper.notebook_test(function() {
    ...
    casper.test.info("Testing selecting report metadata");
    ...
    //We should redirect to tree view. Don't need to test as if this
    //doesn't happen this will wait forever and trigger
    //all the other tests to fail
    ...
    this.then(function tick_then() {
        var array = this.evaluate(function tick_eval() {
            ...
            var dirname = encodeURIComponent("sub ∂ir1");
            var arr = [];
        ...
        if(array.length === 0) {
            ...
            var check_arr = ["sub ∂ir1","file_in_nbdir.txt"];
            var that = this;
    ...
    this.then(function check_ticks_then() {
        ...
        arr = this.evaluate(function check_ticks_eval() {
            ...
            children.each(function () {
                if(this.id === "sub ∂ir1") {
                    stuff.push("sub ∂ir1");
                    if($(this).children().eq(0).attr("id") === 
                       "sub ∂ir1/sub ∂ir 1a")
                    {
                        stuff.push("sub ∂ir 1a");
                        if($(this).children().eq(0).children().eq(0).attr("id") ===
                           "sub ∂ir1/sub ∂ir 1a/file_in_sub_∂ir1a.txt")
                        {
                            stuff.push("file_in_sub_∂ir1a.txt");
                        }
                    } if($(this).children().eq(1).attr("id") ===
                        "sub ∂ir1/file_in_sub_∂ir1.txt") 
                    {
        ...
        that.test.assertEquals(check,5,"All items showing up in the popup");
    });
    ...
    this.then(function redirect_check() {
        ...
        this.test.assertEquals(
            url,
            casper.cli.get("url") + "notebooks/" + nbname,
            "Redirected back to our notebook correctly"
        );
    });
