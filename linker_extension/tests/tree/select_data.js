...
casper.notebook_test(function() {
    ...
    this.then(function check_ticks_then() {
    	...
    	arr = this.evaluate(function check_ticks_eval() {
    		...
    		children.each(function () {
    			if(this.id === "sub ∂ir1") {
    				stuff.push("sub ∂ir1");
    				if($(this).children().eq(0).attr("id") === "sub ∂ir1/sub ∂ir 1a") {
    					stuff.push("sub ∂ir 1a");
    					if($(this).children().eq(0).children().eq(0).attr("id") === "sub ∂ir1/sub ∂ir 1a/file_in_sub_∂ir1a.txt") {
    						stuff.push("file_in_sub_∂ir1a.txt");
    					}
    				} if($(this).children().eq(1).attr("id") === "sub ∂ir1/file_in_sub_∂ir1.txt") {
    ...
    this.then(function redirect_check() {
    	...
    	this.test.assertEquals(url,casper.cli.get("url") + "notebooks/" + nbname,"Redirected back to our notebook correctly");
    });
