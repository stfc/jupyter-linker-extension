casper.notebook_test_at_location(function() {
    ...
    this.then(function tick_then() {
        var array = this.evaluate(function tick_eval() {
            var filename = encodeURIComponent("sub ∂ir1") + "/" + 
                           encodeURIComponent("file_in_sub_∂ir1.txt");

            var dirname = encodeURIComponent("sub ∂ir1") + "/" +
                          encodeURIComponent("sub ∂ir 1a");
            var arr = [];
    ...
    this.then(function redirect_check() {
        ...
        this.test.assertEquals(
            url,
            casper.cli.get("url") + "notebooks/sub%20%E2%88%82ir1/" + nbname,
